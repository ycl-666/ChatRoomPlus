...
Vue.prototype.http2https = function (str) {
    return str.toString().replace("http://", "http://");
};
/**
...
Vue.prototype.request = function (_data) {
    let that = this;
    let url;
    if (_data.url.indexOf("htt") == 0) {
        url = _data.url;
    }
    else {
        url = that.global.apiUrl + "api/" + (_data.url || "");
    }
    axios.post(url, Object.assign({}, that.global.baseData, _data.data || {}), {
        headers: {
